...
- **`DIM_CLIENTE` (spec 06):** `auth/tenantStore.js` hoje é apenas um repasse (`resolverOuProvisionar`/`buscarPorId`) para `db/repositories/clienteRepository.js`, que grava/lê `DIM_CLIENTE` de verdade no Postgres (Sprint 1/Neon) — a nota antiga sobre "Map em memória" já não se aplica.
- **Gestão de usuários real (evolução do RF03):** a tabela operacional `usuarios` (`db/schema.sql`) guarda pessoas com acesso a um `DIM_CLIENTE`, com papel `owner`/`membro`, via `db/repositories/usuarioRepository.js`. O login em `GET /portal/callback` agora decide o destino pelo e-mail: se já existe um `usuarios.email` cadastrado, a sessão resolve para a organização (`id_cliente`) já existente daquele usuário — o nome de escritório digitado no IdP mock é ignorado nesse caso; se o e-mail é novo, provisiona um `DIM_CLIENTE` novo e cria o usuário como `owner`. O dashboard (`public/page.html`) ganhou um ícone de configurações no header que abre um modal (`GET/POST /hub/portal/usuarios`, ambos atrás de `requireSessaoApi`) para o escritório ver e adicionar novos usuários (que entram como `membro`) — sempre lido/escrito do banco, sem estado em memória.
...
- **RF05 (isolamento multi-tenant):** `auth/middleware.js` expõe `requireSessaoApi`/`requireSessaoPagina`, que resolvem `req.clienteId` a partir da sessão — nenhuma rota deve aceitar `id_cliente` vindo do cliente. `GET /portal/whoami` é o exemplo de referência; specs 07/08/09 e a gestão de usuários seguem o mesmo padrão.
- **Testado:** acesso sem sessão (redirect HTML / 401 JSON), fluxo completo de login, dois escritórios distintos resolvendo para `id_cliente` diferentes, `state` forjado rejeitado, `code` de uso único rejeitado no replay, logout, e confirmação de que `/webchat.html` (canal da spec 01) permanece público — sessão só é exigida no portal web. Testado também ponta a ponta contra o Neon: (1) e-mail novo faz bootstrap de organização e vira `owner`; (2) `POST /hub/portal/usuarios` adiciona um segundo usuário como `membro`, e retorna 409 em e-mail duplicado; (3) esse segundo usuário, ao logar via SSO com um nome de escritório *diferente* do digitado no cadastro, entra corretamente na organização já existente (mesmo `id_cliente`, nome do escritório original preservado); (4) ícone de configurações, modal e formulário de adicionar usuário conferidos visualmente via screenshot, incluindo o fluxo completo de adicionar um usuário pela própria UI.
