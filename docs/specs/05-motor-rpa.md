...
- **Script:** [executerRPAEcac.py](../../executerRPAEcac.py) (Playwright `sync_api`, `headless=False`). Mesma política de captcha da spec original (RF06/Fora de Escopo): nunca resolve automaticamente — pausa em `input()` no terminal em cada etapa simulada de captcha/2FA, obrigando confirmação manual de quem está operando (o mesmo padrão se aplicaria a um alvo real, se um dia isso fosse autorizado).
- **Ponte Node → Python:** [robot/consultaEcac.js](../../robot/consultaEcac.js) faz `child_process.spawn` do script com `stdio: ["inherit", "pipe", "pipe"]` — stdin herdado do processo do servidor (as pausas de `input()` são respondidas no terminal onde `node server.js` roda), stdout/stderr capturados linha a linha e repassados ao `Logger` existente, que publica cada etapa via SSE exatamente como o robô da Redesim.
- **Disparo:** menu do webchat, opção `[2] Consulta e-CAC` ([hub/flowEngine.js](../../hub/flowEngine.js)) — como não há documento para OCR, a seleção do serviço já deixa a sessão em `processando` e `hub/router.js` dispara `iniciarConsultaEcac` (novo, em [hub/pipeline.js](../../hub/pipeline.js)) direto, sem esperar upload. Grava em `fato_processamento_automacoes` com `dim_servico_gov.codigo = 'consulta_ecac'` (seed em `db/seed.js`), então aparece no Status Board (spec 07) e no ROI (spec 08) como qualquer outra automação.
- **Testado:** (1) script padrão Playwright validado ponta a ponta contra `public/ecac_fake/` com um script de teste que resolve captcha/2FA automaticamente (só para provar que os seletores/URLs da réplica fictícia batem com os que o robô real usa — a resolução manual em si não foi automatizada no robô de produção); (2) ponte Node↔Python testada isoladamente nos caminhos de sucesso e de erro (`exit code`, leitura de `declaracoes.json`); (3) integração completa via webchat real: seleção do serviço → robô abre o navegador de verdade → navega até a tela de captcha simulada → grava `fato_processamento_automacoes` (`consulta_ecac`) → em ambiente sem terminal interativo (como o usado para este teste automatizado), o `input()` recebe EOF e o erro é corretamente capturado, logado via SSE e persistido como `status = 'erro'` — comportamento esperado; a pausa manual só se resolve com um humano no terminal, que é exatamente o requisito de design.
